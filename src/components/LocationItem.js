...
class LocationItem extends Component {
  ...
  componentDidMount() {
    PostApiService.getPosts().then((resJson) =>
      this.setState({
        posts: resJson,
      })
    );
    ...
    return (
      <div className="location-container-detail">
        <header className="location-item-header">
          <h2 className="location-item-name">
        ...
        <section className="location-item-body">
          <div>
            ...
            <div className="location-detail">{usernotes}</div>
            <a href={url} target="_blank" rel="noopener noreferrer">
              {url}
            </a>
            <div className="post-ratings">
              <div>
                Price: <Rating value={price_rating} />
              </div>
              <div>
                Size: <Rating value={size_rating} />
              </div>
              <div>
                Location: <Rating value={location_rating} />
              </div>
            </div>
            <Link to={`/edit/${id}`}>
